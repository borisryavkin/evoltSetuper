...
int idx = 0;
unsigned long lastPostSpamMs = 0;

const unsigned long kPostSpamIntervalMs = 200; // match ESC spam cadence
const unsigned long kPostSpamGapMs = 50;       // small gap between y and Enter
...
void loop() {
  if (idx >= N) {
    ...
    if (now - lastPostSpamMs >= kPostSpamIntervalMs) {
      Keyboard.write('y');
      delay(kPostSpamGapMs);
